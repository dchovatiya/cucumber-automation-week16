    ...
    public void allFieldNames(String fieldName, String sendValue) {
        ...
        for (WebElement selField : selFields) {
            if (selField.getAttribute("name").trim().equalsIgnoreCase(fieldName)) {
               ...
               /* if (fieldName.equalsIgnoreCase("id_state") || fieldName.equalsIgnoreCase("id_country")) {
                    selectByVisibleTextFromDropDown(selField, sendValue);
                } else {
                    selectByValueFromDropDown(selField, sendValue);
                }*/
                break;
            }
